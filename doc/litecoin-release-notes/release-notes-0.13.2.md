Hatchcoin Core version 0.13.2 is now available from:

  <https://download.hatchcoin.org/hatchcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/hatchcoin-project/hatchcoin/issues>
...
updates will be released anymore. Without security updates, using a hatchcoin
...
ECDSA signatures inside Hatchcoin transactions now use validation using
...
Previous versions of Hatchcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Hatchcoin Core was to raise the default minimum
relay fee.

Hatchcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Hatchcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running hatchcoind without having to do any manual configuration.
...
Hatchcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Hatchcoin Core will relay transactions with
...
entirely in Hatchcoin Core 0.13, and it is at this time undecided whether the
...
Hatchcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Hatchcoin Core automatically creates a hidden service to listen on, without
manual configuration. Hatchcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Hatchcoin Core is listening, and
...
Hatchcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Hatchcoin
Core to use floating fees. By default, Hatchcoin Core will use floating
fees.
...
(default: `0.0002` HTC/kB).

At all times, Hatchcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HTC.
Furthermore, Hatchcoin Core will never create transactions paying less than
...
`hatchcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /hatchcoinrpc>
        ...
        # optional bypass hatchcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< hatchcoinrpc:<password>
    </Location>
...
Restarting hatchcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in hatchcoin.conf
...
hatchcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/hatchcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Hatchcoin Core code base has
...
- `hatchcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `hatchcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Hatchcoin scripting system that in combination with
...
Hatchcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Hatchcoin Core.
...
The two phases are distinct in the Hatchcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Hatchcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Hatchcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Hatchcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of hatchcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of hatchcoins being spent is exactly
...
- **Increased security for multisig:** Hatchcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Hatchcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Hatchcoin's current ledger (called the UTXO set),
  ...
  note that Hatchcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Hatchcoin users to individually opt-in to almost any change in the Hatchcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Hatchcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Hatchcoin
client to Hatchcoin Core 0.13.2 to signal for segwit support. When a
...
Since Hatchcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `hatchcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `hatchcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
