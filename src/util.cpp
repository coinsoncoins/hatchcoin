...
const char * const BITCOIN_CONF_FILENAME = "hatchcoin.conf";
const char * const BITCOIN_PID_FILENAME = "hatchcoin.pid";
...
#else
    const char* pszModule = "hatchcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hatchcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hatchcoin";
#else
    // Unix
    return pathRet / ".hatchcoin";
#endif
#endif
}
